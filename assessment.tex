  ...
  \section*{Instructions}
    ...
    \begin{enumerate}
      ...
      \item
        ...
        However, Microsoft Excel has two different versions of the standard deviation calculation, \mintinline{python}{STDDEV.P} and \mintinline{python}{STDDEV.S}.
        The \mintinline{python}{STDDEV.P} function performs the above calculation but in the \mintinline{python}{STDDEV.S} calculation the division is by \mintinline{python}{len(x)-1} rather than \mintinline{python}{len(x)}.
        Research these Excel functions, writing a note in a Markdown cell about the difference between them.
        Then use \mintinline{python}{numpy} to perform a simulation demonstrating that the \mintinline{python}{STDDEV.S} calculation is a better estimate for the standard deviation of a population when performed on a sample.
